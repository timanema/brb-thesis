...
% In order to build a full routing table based on our disjoint path solver, one only has to vary the $t$ parameter, which is essentially the target process. The full table can be build by iterating over all processes for $t$.
...
\begin{figure}[!htbp]
    \begin{floatrow}
        ...
        {
            \caption{Both blue and red paths can be dropped, as only one direct path to a neighbour is required}
            \label{contr:ord2-ex}
        }
    \end{floatrow}
\end{figure}
