...
\jd{not a distributed model, but the one you use.}

A distributed model is defined by a set $\Delta=\{p_1, p_2,...,p_N\}$ of N processes, uniquely identified by an ID $i$. In the Byzantine fault-model it is assumed that there are at most $f < \floor{N/3}$ Byzantine nodes, which can exhibit arbitrary behaviour. \jd{processes know their ID? the ones of the others? Do they know their neighborhood?}
...
This works by layering the two protocols, where Dolev's protocol forms the bottom layer. This means \jd{that} every Bracha broadcast is replaced by a Dolev broadcast, and every Bracha receive by a Dolev deliver. This process is shown in figure~\ref{background:bracha-dolev}.
...
\jd{the paper describes a routed version of Dolev. However, it is not said how these routes should be computed.}
