...
When a network is partially connected, Dolev showed that processes can still communicate reliably in the presence of $f$ Byzantine nodes when the network is at least $2f+1$-connected~\cite{dolev}. In this solution a message is flooded over the network for the unknown version, therefore following at least $2f+1$ vertex-disjoint paths. In the routed version messages are only sent over there predesignated routes. 
